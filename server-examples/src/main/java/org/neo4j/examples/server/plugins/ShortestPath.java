    ...
    public Iterable<Path> shortestPath(
        ...
        if (types == null) {
            ...
            for (String type : types) {
                expanderBuilder = expanderBuilder.add(RelationshipType.withName(type));
            }
            expander = expanderBuilder.build();
        }
        try (Transaction tx = source.getGraphDatabase().beginTx()) {
            PathFinder<Path> shortestPath = GraphAlgoFactory.shortestPath(expander, depth == null ? 4 : depth);
