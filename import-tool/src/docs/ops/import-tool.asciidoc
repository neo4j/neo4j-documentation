...
Under Unix/Linux/OSX, the command is named `neo4j-admin import`. Depending on the installation type, the tool is either available globally, or used by executing `./bin/neo4j-admin import` from inside the installation directory.
