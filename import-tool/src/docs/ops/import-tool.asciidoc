...
Instead you'll need to add indexes afterwards (see <<graphdb-neo4j-indexes>>).
...
Under Unix/Linux/macOS, the command is named `neo4j-admin import`. Depending on the installation type, the tool is either available globally, or used by executing `./bin/neo4j-admin import` from inside the installation directory.
