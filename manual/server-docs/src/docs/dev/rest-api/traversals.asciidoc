...
:server-extending-uri: {java-reference-base-uri}/#server-extending
:operations-security-uri: {java-reference-base-uri}/#security-rest-api-authorization-rules

[WARNING]
--
The Traversal REST Endpoint executes arbitrary Javascript code under the hood as part of the evaluators definitions.
In hosted and open environments, this may constitute a security risk.
In these case, consider using declarative approaches like link:{cypher-query-lang-uri}[Cypher] or write your own server side extension executing the interesting traversals with the Java API (see link:{server-extending-uri}[Extending Neo4j]).
To properly secure your server, see link:{operations-security-uri}[Operations Manual -> Security].
--
...
uniqueness:: Decides how uniqueness should be calculated.
 For details on different uniqueness values see the link:{java-reference-base-uri}/javadocs/org/neo4j/graphdb/traversal/Uniqueness.html[Java API on Uniqueness].
 Possible values:
 * +node_global+
 * +none+
 * +relationship_global+
...
The script body will be executed in a Java context which has access to the full Neo4j link:{java-reference-base-uri}/javadocs/[Java API].
