...
[CAUTION]
--
This is a sharp tool, allowing users to deploy arbitrary https://en.wikipedia.org/wiki/JAX-RS[JAX-RS] classes to the server so be careful when using this.
...
https://github.com/neo4j/neo4j-documentation/blob/{neo4j-git-tag}/manual/server-examples/src/main/java/org/neo4j/examples/server/unmanaged/HelloWorldResource.java[HelloWorldResource.java]
...
We also need to tell Neo4j where to look for the extension by adding some configuration in _neo4j.conf_:
