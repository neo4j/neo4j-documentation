...
You can also use the link:{cypher-query-lang-uri}[Cypher query language] as a powerful declarative way to query the graph.
...
link:javadocs/org/neo4j/graphdb/traversal/TraversalDescription.html#relationships-org.neo4j.graphdb.RelationshipType-org.neo4j.graphdb.Direction-[including direction] and another one
link:javadocs/org/neo4j/graphdb/traversal/TraversalDescription.html#relationships-org.neo4j.graphdb.RelationshipType-[excluding direction], where the latter traverses relationships in
link:javadocs/org/neo4j/graphdb/Direction.html#BOTH[both directions].
...
These are convenience methods for setting preorder https://en.wikipedia.org/wiki/Depth-first_search[depth-first]/ https://en.wikipedia.org/wiki/Breadth-first_search[breadth-first] `BranchSelector`|`ordering` policies.
The same result can be achieved by calling the link:javadocs/org/neo4j/graphdb/traversal/TraversalDescription.html#order-org.neo4j.graphdb.traversal.BranchOrderingPolicy-[+order+] method with ordering policies from link:javadocs/org/neo4j/graphdb/traversal/BranchOrderingPolicies.html[+BranchOrderingPolicies+], or to write your own +BranchSelector+/+BranchOrderingPolicy+ and pass in.
...
In essence these are a composite of a Path and a RelationshipExpander that can be used to get new link:javadocs/org/neo4j/graphdb/traversal/TraversalBranch.html[+TraversalBranch+]es from the current one.
...
include::examples/how-to-use-the-traversal-framework.asciidoc[]
