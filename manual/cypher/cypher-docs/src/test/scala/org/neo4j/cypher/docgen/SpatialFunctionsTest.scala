...
class SpatialFunctionsTest extends DocumentingTest {
  ...
  override def doc = new DocBuilder {
    ...
    p(
      """_WGS 84_ is specified with a map containing coordinate values for either `longitude` and `latitude` (this is the default), or `x` and `y`.
    ...
    graphViz()
      section("point() - WGS 84", "functions-point") {
        ...
        function("`point({longitude | x, latitude | y [, crs]})`", ("A single map consisting of the following", ""), ("longitude/x", "A numeric expression"),
          ("latitude/y", "A numeric expression"), ("crs", "The string 'WGS-84'"))
        query("RETURN point({longitude: 56.7, latitude: 12.78}) AS point",
          ResultAssertions((r) => {
        ...
        query("RETURN point({x: 2.3, y: 4.5, crs: 'WGS-84'}) AS point",
          ResultAssertions((r) => {
      ...
      section("point() - cartesian 2D", "functions-point-cartesian") {
        ...
        function("`point({x, y [, crs]})`", ("A single map consisting of the following", ""), ("x", "A numeric expression"),
          ("y", "A numeric expression"), ("crs", "The string 'cartesian'"))
        query("RETURN point({x: 2.3, y: 4.5}) AS point",
          ResultAssertions((r) => {
      ...
      section("distance()", "functions-distance") {
        ...
        query(
          """MATCH (t:TrainStation)-[:TRAVEL_ROUTE]->(o:Office)
            ...
            |RETURN round(distance(trainPoint, officePoint)) AS travelDistance""".stripMargin,
          ResultAssertions((r) => {
