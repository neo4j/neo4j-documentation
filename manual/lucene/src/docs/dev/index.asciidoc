...
// NOTE: This is not the same as <<graphdb-neo4j-schema, indexes defined in the schema>>, the documentation below is for the manual indexing in Neo4j.
...
Manual indexing operations are part of the link:javadocs/org/neo4j/graphdb/index/package-summary.html[Neo4j index API].
...
Calls made to such an index instance after link:javadocs/org/neo4j/graphdb/index/Index.html#delete--[delete()] has been called are invalid inside that transaction as well as outside (if the transaction is successful), but will become valid again if the transaction is rolled back.
...
link:javadocs/org/neo4j/graphdb/index/Index.html#remove-T-java.lang.String-java.lang.Object-[Removing] from an index is similar to adding, but can be done by supplying one of the following combinations of arguments:
...
An index can be searched in two ways, link:javadocs/org/neo4j/graphdb/index/ReadableIndex.html#get-java.lang.String-java.lang.Object-[get] and link:javadocs/org/neo4j/graphdb/index/ReadableIndex.html#query-java.lang.Object-[query].
...
For example the {lucene-base-uri}/queryparser/org/apache/lucene/queryparser/classic/package-summary.html#package_description[Lucene query syntax] can be used directly with the default indexing backend.
...
link:javadocs/org/neo4j/graphdb/index/IndexHits.html[IndexHits] is an `Iterable` with some additional useful methods.
For example link:javadocs/org/neo4j/graphdb/index/IndexHits.html#getSingle--[getSingle()] returns the first and only item from the result iterator, or `null` if there isn't any hit.
...
IMPORTANT: In case you don't iterate through all the hits, link:javadocs/org/neo4j/graphdb/index/IndexHits.html#close--[IndexHits.close()] must be called explicitly.
...
The `IndexHits` interface exposes link:javadocs/org/neo4j/graphdb/index/IndexHits.html#currentScore--[scoring] so that the index can communicate scores for the hits.
...
----
component=neo4j-lucene-index-docs
source=org/neo4j/index/impl/lucene/legacy/TestLuceneIndex.java
...
| type | `exact`, `fulltext` |
  `exact` is the default and uses a Lucene  {lucene-base-uri}/analyzers-common/org/apache/lucene/analysis/core/KeywordAnalyzer.html[keyword analyzer].
...
| analyzer | the full class name of an {lucene-base-uri}/core/org/apache/lucene/analysis/Analyzer.html[Analyzer] | Overrides the `type` so that a custom analyzer can be used.
...
Note that the {lucene-base-uri}/core/org/apache/lucene/search/TermQuery.html[TermQuery] is basically the same thing as using the `get` method on the index.
