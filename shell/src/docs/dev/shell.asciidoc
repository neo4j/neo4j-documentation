...
[NOTE]
--
This command does not list explicit indexes.
For working with explicit indexes, please see <<shell-explicit-indexing>>.
--
...
[[shell-explicit-indexing]]
=== Explicit Indexing

It's possible to query and manipulate explicit indexes via the index command.

Example: +index -i persons name+ (will index the name for the current node or relationship in the "persons" explicit index).

* +-g+ will do exact lookup in the explicit index and display hits. You can supply +-c+ with a command to be executed for each hit.
* +-q+ will ask the explicit index a query and display hits. You can supply +-c+ with a command to be executed for each hit.
...
* +-i+ will index a key-value pair into a explicit index for the current node/relationship. If no value is given the property value for that key for the current node is used as value.
* +-r+ will remove a key-value pair (if it exists) from a explicit index for the current node/relationship. Key and value are optional.
* +-t+ will set the explicit index type to work with, for example `index -t Relationship --delete friends` will delete the +friends+ relationship index.
