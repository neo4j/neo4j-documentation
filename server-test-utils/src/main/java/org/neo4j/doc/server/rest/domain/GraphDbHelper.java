...
public class GraphDbHelper
{
    ...
    public int getNumberOfNodes() {
        ...
        try (Session session = kernel.beginSession( AnonymousContext.read() );
             org.neo4j.internal.kernel.api.Transaction tx = session.beginTransaction( implicit ) )
        {
    ...
    {
        try (Transaction tx = database.getGraph().beginTransaction( implicit, SecurityContext.AUTH_DISABLED ))
        {
    ...
    {
        try ( Transaction transaction = database.getGraph().beginTransaction( implicit, SecurityContext.AUTH_DISABLED ) )
        {
