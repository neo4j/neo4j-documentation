...
class ImmutablePrivilegeTest extends AdministrationCommandTestBase {
  val title = "(★) Immutable privileges"

  override val linkId =
    "administration/security/administration/#access-control-privileges-immutable"

  private def setup() = graph.withTx { tx =>
    tx.execute("CREATE ROLE my_role")
    tx.execute("GRANT IMMUTABLE DATABASE MANAGEMENT ON DBMS TO my_role")
  }
