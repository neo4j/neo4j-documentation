...
class SetTest extends DocumentingTest with QueryStatisticsTestSupport {
  ...
  override def doc = new DocBuilder {
    ...
    section("Set a property", "set-set-a-property") {
      p("""Use `SET` to set a property on a node or relationship:""")
      query("""MATCH (n {name: 'Andy'})
              #SET n.surname = 'Taylor'
              #RETURN n.name, n.surname""".stripMargin('#'),
      ResultAssertions((r) => {
      ...
      p("""It is possible to set a property on a node or relationship using more complex expressions.
      ...
      query("""MATCH (n {name: 'Andy'})
              ...
              #RETURN n.name, n.worksIn""".stripMargin('#'),
      ResultAssertions((r) => {
      ...
      p("""No action will be taken if the node expression evaluates to `null`, as shown in this example:""")
      query("""MATCH (n {name: 'Andy'})
      ...
      ResultAssertions((r) => {
        ...
        p("""As no node matches the `CASE` expression, the expression returns a `null`.
    ...
    section("Update a property", "set-update-a-property") {
      p("""`SET` can be used to update a property on a node or relationship.
    ...
    section("Remove a property", "set-remove-a-property") {
      p("""Although `<<query-remove, REMOVE>>` is normally used to remove a property, it's sometimes convenient to do it using the `SET` command.
    ...
    section("Replace all properties using a map and `=`", "set-replace-properties-using-map") {
      p("""The property replacement operator `=` can be used with `SET` to replace all existing properties on a node or relationship with those provided by a map:""")
      query("""MATCH (p {name: 'Peter'})
              #SET p = {name: 'Peter Smith', position: 'Entrepreneur'}
              #RETURN p.name, p.age, p.position""".stripMargin('#'),
      ResultAssertions((r) => {
    ...
    section("Remove all properties using an empty map and `=`", "set-remove-properties-using-empty-map") {
      p("""All existing properties can be removed from a node or relationship by using `SET` with `=` and an empty map as the right operand:""")
      query("""MATCH (p {name: 'Peter'})
              #SET p = {}
              #RETURN p.name, p.age""".stripMargin('#'),
      ResultAssertions((r) => {
    ...
    section("Mutate specific properties using a map and `+=`", "set-setting-properties-using-map") {
      p("""The property mutation operator `+=` can be used with `SET` to mutate properties from a map in a fine-grained fashion:
          #
      ...
      query("""MATCH (p {name: 'Peter'})
              ...
              #RETURN p.name, p.age, p.hungry, p.position""".stripMargin('#'),
      ResultAssertions((r) => {
      ...
      p("""<<set-remove-properties-using-empty-map, In contrast to the property replacement operator `=`>>, providing an empty map as the right operand to `+=` will not remove any existing properties from a node or relationship.
    ...
    section("Set multiple properties using one `SET` clause", "set-set-multiple-properties-using-one-set-clause") {
      p("""Set multiple properties at once by separating them with a comma:""")
      query("""MATCH (n {name: 'Andy'})
              #SET n.position = 'Developer', n.surname = 'Taylor'""".stripMargin('#'),
      ResultAssertions((r) => {
          assertStats(r, propertiesWritten = 2, nodesCreated = 0)
        })){
        resultTable()
    ...
    section("Set a property using a parameter", "set-set-a-property-using-a-parameter") {
      p("""Use a parameter to set the value of a property:""")
      query("""MATCH (n {name: 'Andy'})
    ...
    section("Set all properties using a parameter", "set-set-all-properties-using-a-parameter") {
      p("""This will replace all existing properties on the node with the new set provided by the parameter.""")
      query("""MATCH (n {name: 'Andy'})
              #SET n = $props
              #RETURN n.name, n.position, n.age, n.hungry""".stripMargin('#'),
      ResultAssertions((r) => {
    ...
    section("Set a label on a node", "set-set-a-label-on-a-node") {
      p("""Use `SET` to set a label on a node:""")
      query("""MATCH (n {name: 'Stefan'})
              #SET n:German
              #RETURN n.name, labels(n) AS labels""".stripMargin('#'),
      ResultAssertions((r) => {
    ...
    section("Set multiple labels on a node", "set-set-multiple-labels-on-a-node") {
      p("""Set multiple labels on a node with `SET` and use `:` to separate the different labels:""")
      query("""MATCH (n {name: 'George'})
