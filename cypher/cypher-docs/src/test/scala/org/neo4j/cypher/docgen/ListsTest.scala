  ...
  override def doc = new DocBuilder {
    ...
    section("List comprehension", "cypher-list-comprehension") {
      ...
      query(
        ...
        ResultAssertions((r) => {
          r.toList should equal(List(Map("result" -> List(0.0, 1.0, 8.0, 27.0, 64.0, 125.0, 216.0, 343.0, 512.0, 729.0,
            1000.0))))
        })
      ) {
        resultTable()
    ...
    section("Pattern comprehension", "cypher-pattern-comprehension") {
      ...
      query(
        """MATCH (a:Person {name: 'Keanu Reeves'})
          #RETURN [(a)-->(b:Movie) WHERE b.title CONTAINS 'Matrix' | b.released] AS years""".stripMargin('#'),
        ResultAssertions((r) => {
      ...
      query(
        """MATCH (a:Person {name: 'Keanu Reeves'})
          ...
          #RETURN COLLECT(year) AS sorted_years""".stripMargin('#'),
        ResultAssertions((r) => {
