  ...
  override def doc = new DocBuilder {
    ...
    section("Temporal operators", "query-operators-temporal") {
      ...
      section("Multiplying and dividing a _Duration_ with or by a number", "syntax-multiply-divide-duration-number") {
        p(
          """These operations are interpreted simply as component-wise operations with overflow to smaller units based on an average length of units in the case of division (and multiplication with fractions).""".stripMargin
        )
        query(
    ...
    section("List operators", "query-operators-list") {
      ...
      section(
        ...
        p(
          """The following query checks whether or not the list `[2, 1]` is an element of the list `[1, [2, 1], 3]`:""".stripMargin
        )
        query(
