...
class TemporalFunctionsTest extends DocumentingTest {
  ...
  override def doc = new DocBuilder {
    ...
    section("Temporal instant types", "functions-temporal-instant-type") {
      synopsis(
        """An introduction to temporal instant types, including descriptions of creation functions, clocks, and truncation.""".stripMargin
      )
    ...
    section("Date: `date()`", "functions-date") {
      ...
      section("Getting the current _Date_", "functions-date-current") {
        ...
        query(
          """RETURN date() AS currentDate""".stripMargin,
          ResultAssertions((r) => {
        ...
        query(
          """RETURN date( {timezone: 'America/Los Angeles'} ) AS currentDateInLA""".stripMargin,
          ResultAssertions((r) => {
        ...
        section("date.transaction()", "functions-date-transaction") {
          ...
          query(
            """RETURN date.transaction() AS currentDate""".stripMargin,
            ResultAssertions((r) => {
              val now = r.columnAs[LocalDate]("currentDate").next()
              now should be(a[LocalDate])
            })
      ...
      section("Creating a calendar (Year-Month-Day) _Date_", "functions-date-calendar") {
        ...
        function(
          ...
          (
            "year",
            "An expression consisting of at <<cypher-temporal-year, least four digits>> that specifies the year."
          ),
        ...
        query(
          """UNWIND [
            ...
            |  date({year:1984})
            |] as theDate
            |RETURN theDate""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("theDate" -> LocalDate.of(1984, 10, 11)),
              Map("theDate" -> LocalDate.of(1984, 10, 1)),
              Map("theDate" -> LocalDate.of(1984, 1, 1))
            ))
          })
        ) {
          resultTable()
      ...
      section("Creating a week (Year-Week-Day) _Date_", "functions-date-week") {
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("theDate" -> LocalDate.of(1984, 3, 7)),
              Map("theDate" -> LocalDate.of(1984, 3, 5)),
              Map("theDate" -> LocalDate.of(1984, 1, 1))
            ))
          })
        ) {
      ...
      section("Creating a quarter (Year-Quarter-Day) _Date_", "functions-date-quarter") {
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("theDate" -> LocalDate.of(1984, 8, 14)),
              Map("theDate" -> LocalDate.of(1984, 7, 1)),
              Map("theDate" -> LocalDate.of(1984, 1, 1))
            ))
          })
        ) {
      ...
      section("Creating an ordinal (Year-Day) _Date_", "functions-date-ordinal") {
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("theDate" -> LocalDate.of(1984, 7, 20)),
              Map("theDate" -> LocalDate.of(1984, 1, 1))
            ))
          })
        ) {
      ...
      section("Creating a _Date_ using other temporal values as components", "functions-date-temporal") {
        ...
        query(
          """UNWIND [
            ...
            |   date({date: dd, day: 28}) AS dateDay""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map(
                "dateOnly" -> DateValue.parse("1984-11-11").asObjectCopy(),
                "dateDay" -> DateValue.parse("1984-11-28").asObjectCopy()
              ),
              Map(
                "dateOnly" -> DateValue.parse("1984-11-11").asObjectCopy(),
                "dateDay" -> DateValue.parse("1984-11-28").asObjectCopy()
      ...
      section("Truncating a _Date_", "functions-date-truncate") {
        ...
        query(
          """WITH datetime({year:2017, month:11, day:11, hour:12, minute:31, second:14, nanosecond: 645876123, timezone: '+01:00'}) AS d
            ...
            |       date.truncate('day', d) AS truncDay""".stripMargin,
          ResultAssertions((r) => {
    ...
    section("DateTime: `datetime()`", "functions-datetime") {
      ...
      section("Getting the current _DateTime_", "functions-datetime-current") {
        ...
        section("datetime.realtime()", "functions-datetime-realtime") {
          ...
          query(
            """RETURN datetime.realtime() AS currentDateTime""".stripMargin,
            ResultAssertions((r) => {
              val now = r.columnAs[ZonedDateTime]("currentDateTime").next()
              now should be(a[ZonedDateTime])
            })
      ...
      section("Creating a calendar (Year-Month-Day) _DateTime_", "functions-datetime-calendar") {
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("theDate" -> DateTimeValue.parse(
      ...
      section("Creating a _DateTime_ using other temporal values as components", "functions-datetime-temporal") {
        ...
        query(
          """WITH date({year:1984, month:10, day:11}) AS dd
            ...
            |       datetime({date:dd, day: 28, hour: 10, minute: 10, second: 10, timezone:'Pacific/Honolulu'}) AS dateDDHHMMSSTimezone""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
              "dateHHMMSS" -> DateTimeValue.parse("1984-10-11T10:10:10Z", defaultZoneSupplier).asObjectCopy(),
              "dateHHMMSSTimezone" -> DateTimeValue.parse(
                "1984-10-11T10:10:10+05:00",
                defaultZoneSupplier
              ).asObjectCopy(),
              "dateDDHHMMSS" -> DateTimeValue.parse("1984-10-28T10:10:10Z", defaultZoneSupplier).asObjectCopy(),
              "dateDDHHMMSSTimezone" -> DateTimeValue.parse(
                "1984-10-28T10:10:10[Pacific/Honolulu]",
                defaultZoneSupplier
              ).asObjectCopy()
            )))
          })
        ...
        query(
          """WITH time({hour:12, minute:31, second:14, microsecond: 645876, timezone: '+01:00'}) AS tt
            ...
            |       datetime({year:1984, month:10, day:11, time:tt, second: 42, timezone:'Pacific/Honolulu'}) AS YYYYMMDDTimeSSTimezone""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
              "YYYYMMDDTime" -> DateTimeValue.parse(
                "1984-10-11T12:31:14.645876+01:00",
                defaultZoneSupplier
              ).asObjectCopy(),
              "YYYYMMDDTimeTimezone" -> DateTimeValue.parse(
                "1984-10-11T16:31:14.645876+05:00",
                defaultZoneSupplier
              ).asObjectCopy(),
              "YYYYMMDDTimeSS" -> DateTimeValue.parse(
                "1984-10-11T12:31:42.645876+01:00",
                defaultZoneSupplier
              ).asObjectCopy(),
              "YYYYMMDDTimeSSTimezone" -> DateTimeValue.parse(
        ...
        query(
          """WITH datetime({year:1984, month:10, day:11, hour:12, timezone: 'Europe/Stockholm'}) AS dd
            ...
            |       datetime({datetime:dd, day: 28, second: 42, timezone:'Pacific/Honolulu'}) AS dateTimeDDSSTimezone""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
              "dateTime" -> DateTimeValue.parse(
              ...
              "dateTimeDDSS" -> DateTimeValue.parse(
      ...
      section("Creating a _DateTime_ from a timestamp", "functions-datetime-timestamp") {
        ...
        query(
          """RETURN datetime({epochSeconds:timestamp() / 1000, nanosecond: 23}) AS theDate""".stripMargin,
          ResultAssertions((r) => {
        ...
        query(
          """RETURN datetime({epochMillis: 424797300000}) AS theDate""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(
              List(Map("theDate" -> DateTimeValue.ofEpochMillis(Values.longValue(424797300000L)).asObjectCopy()))
            )
          })
        ) {
          resultTable()
      ...
      section("Truncating a _DateTime_", "functions-datetime-truncate") {
        ...
        query(
          """WITH datetime({year:2017, month:11, day:11, hour:12, minute:31, second:14, nanosecond: 645876123, timezone: '+03:00'}) AS d
            ...
            |       datetime.truncate('second', d) AS truncSecond""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
              "truncMillenium" -> DateTimeValue.parse(
                "2000-01-01T00:00[Europe/Stockholm]",
                defaultZoneSupplier
              ).asObjectCopy(),
    ...
    section("LocalDateTime: `localdatetime()`", "functions-localdatetime") {
      ...
      section("Creating a calendar (Year-Month-Day) _LocalDateTime_", "functions-localdatetime-calendar") {
        ...
        query(
          """RETURN localdatetime({year:1984, month:10, day:11, hour:12, minute:31, second:14, millisecond: 123, microsecond: 456, nanosecond: 789}) AS theDate""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(
              List(Map("theDate" -> LocalDateTimeValue.parse("1984-10-11T12:31:14.123456789").asObjectCopy()))
            )
          })
        ) {
          resultTable()
      ...
      section("Creating a week (Year-Week-Day) _LocalDateTime_", "functions-localdatetime-week") {
        ...
        query(
          """RETURN localdatetime({year:1984, week:10, dayOfWeek:3, hour:12, minute:31, second:14, millisecond: 645}) AS theDate""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(
              List(Map("theDate" -> LocalDateTimeValue.parse("1984-03-07T12:31:14.645").asObjectCopy()))
            )
          })
        ) {
          resultTable()
      ...
      section("Creating a quarter (Year-Quarter-Day) _DateTime_", "functions-localdatetime-quarter") {
        ...
        query(
          """RETURN localdatetime({year:1984, quarter:3, dayOfQuarter: 45, hour:12, minute:31, second:14, nanosecond: 645876123}) AS theDate""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(
              List(Map("theDate" -> LocalDateTimeValue.parse("1984-08-14T12:31:14.645876123").asObjectCopy()))
            )
          })
        ) {
          resultTable()
      ...
      section("Creating an ordinal (Year-Day) _LocalDateTime_", "functions-localdatetime-ordinal") {
        ...
        query(
          """RETURN localdatetime({year:1984, ordinalDay:202, hour:12, minute:31, second:14, microsecond: 645876}) AS theDate""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(
              List(Map("theDate" -> LocalDateTimeValue.parse("1984-07-20T12:31:14.645876").asObjectCopy()))
            )
          })
        ) {
          resultTable()
      ...
      section(
        ...
        query(
          """WITH date({year:1984, month:10, day:11}) AS dd
            ...
            |       localdatetime({date:dd, day: 28, hour: 10, minute: 10, second: 10}) AS dateDDHHMMSS""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
        ...
        query(
          """WITH time({hour:12, minute:31, second:14, microsecond: 645876, timezone: '+01:00'}) AS tt
            ...
            |       localdatetime({year:1984, month:10, day:11, time:tt, second: 42}) AS YYYYMMDDTimeSS""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
        ...
        query(
          """WITH date({year:1984, month:10, day:11}) AS dd,
            ...
            |       localdatetime({date:dd, time:tt, day: 28, second: 42}) AS dateTimeDDSS""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
        ...
        query(
          """WITH datetime({year:1984, month:10, day:11, hour:12, timezone: '+01:00'}) as dd
            ...
            |       localdatetime({datetime:dd, day: 28, second: 42}) as dateTimeDDSS""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
      ...
      section("Truncating a _LocalDateTime_", "functions-localdatetime-truncate") {
        ...
        query(
          """WITH localdatetime({year:2017, month:11, day:11, hour:12, minute:31, second:14, nanosecond: 645876123}) AS d
            ...
            |       localdatetime.truncate('second', d) AS truncSecond""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
    ...
    section("LocalTime: `localtime()`", "functions-localtime") {
      ...
      section("Getting the current _LocalTime_", "functions-localtime-current") {
        ...
        section("localtime.statement()", "functions-localtime-statement") {
          ...
          query(
            """RETURN localtime.statement('America/Los Angeles') AS nowInLA""".stripMargin,
            ResultAssertions((r) => {
              val now = r.columnAs[LocalTime]("nowInLA").next()
              now should be(a[LocalTime])
            })
      ...
      section("Creating a _LocalTime_ using other temporal values as components", "functions-localtime-temporal") {
        ...
        query(
          """WITH time({hour:12, minute:31, second:14, microsecond: 645876, timezone: '+01:00'}) AS tt
            ...
            |       localtime({time:tt, second: 42}) AS timeSS""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
      ...
      section("Truncating a _LocalTime_", "functions-localtime-truncate") {
        ...
        query(
          """WITH time({hour:12, minute:31, second:14, nanosecond: 645876123, timezone: '-01:00'}) AS t
            ...
            |       localtime.truncate('microsecond', t) AS truncMicrosecond""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
      ...
      section("Creating a _Time_ using other temporal values as components", "functions-time-temporal") {
        ...
        query(
          """WITH localtime({hour:12, minute:31, second:14, microsecond: 645876}) AS tt
            ...
            |       time({time:tt, second: 42, timezone:'+05:00'}) AS timeSSTimezone""".stripMargin,
          ResultAssertions((r) => {
            r.toList should equal(List(Map(
