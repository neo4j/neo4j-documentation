...
class UnwindTest extends DocumentingTest {
  ...
  override def doc = new DocBuilder {
    ...
    p("""* <<query-unwind-introduction, Introduction>>
    ...
    section("Unwinding a list", "unwind-unwinding-a-list") {
      p("""We want to transform the literal list into rows named `x` and return them.""")
      query(
    ...
    section("Creating a distinct list", "unwind-creating-a-distinct-list") {
      p("""We want to transform a list of duplicates into a set using `DISTINCT`.""")
      query(
    ...
    section(
      ...
      p("""Any expression that returns a list may be used with `UNWIND`.""")
      query(
        """WITH
          #  [1, 2] AS a,
          #  [3, 4] AS b
    ...
    section("Using `UNWIND` with a list of lists", "unwind-using-unwind-with-a-list-of-lists") {
      ...
      query(
        ...
        p("""The first `UNWIND` results in three rows for `x`, each of which contains an element of the original list (two of which are also lists); namely, `[1, 2]`, `[3, 4]` and `5`.
    ...
    section("Using `UNWIND` with an empty list", "unwind-using-unwind-with-an-empty-list") {
      ...
      p("""Essentially, `UNWIND []` reduces the number of rows to zero, and thus causes the query to cease its execution, returning no results.
    ...
    section(
      ...
      p("""Using `UNWIND` on an expression that does not return a list, will return the same result as using `UNWIND` on a list that just contains that expression.
