  ...
  override def doc: Document = new DocBuilder {
    ...
    section("Basic node finding", "basic-node-finding") {
      ...
      section("Match with a label expression for the node labels", "syntax-or-label") {
        ...
        query(
          """MATCH (n:Movie|Person) RETURN n.name AS name, n.title AS title""".stripMargin('#'),
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("name" -> "Charlie Sheen", "title" -> null),
              Map("name" -> "Martin Sheen", "title" -> null),
              Map("name" -> "Michael Douglas", "title" -> null),
              Map("name" -> "Oliver Stone", "title" -> null),
              Map("name" -> "Rob Reiner", "title" -> null),
              Map("name" -> null, "title" -> "Wall Street"),
              Map("name" -> null, "title" -> "The American President")
            ))
          })
        ) {
          resultTable()
