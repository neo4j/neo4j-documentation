...
class SpatialFunctionsTest extends DocumentingTest {
  ...
  override def doc = new DocBuilder {
    ...
    section("point.distance()", "functions-distance") {
      ...
      query(
        """WITH point({x: 2.3, y: 4.5, crs: 'cartesian'}) AS p1, point({x: 1.1, y: 5.4, crs: 'cartesian'}) AS p2
          |RETURN point.distance(p1,p2) AS dist""".stripMargin,
        ResultAssertions((r) => {
      ...
      query(
        """MATCH (t:TrainStation)-[:TRAVEL_ROUTE]->(o:Office)
          ...
          |RETURN round(point.distance(trainPoint, officePoint)) AS travelDistance""".stripMargin,
        ResultAssertions((r) => {
    ...
    section("point.withinBBox()", "functions-withinBBox") {
      ...
      query(
        """WITH point({x: 0, y: 0, crs: 'cartesian'}) AS lowerLeft, point({x: 10, y: 10, crs: 'cartesian'}) AS upperRight
          |RETURN point.withinBBox(point({x: 5, y: 5, crs: 'cartesian'}), lowerLeft, upperRight) AS result""".stripMargin,
        ResultAssertions((r) => {
    ...
    section("point() - WGS 84 2D", "functions-point-wgs84-2d") {
      ...
      query(
        """MATCH (p:Office)
          |RETURN point({longitude: p.longitude, latitude: p.latitude}) AS officePoint""".stripMargin,
        ResultAssertions((r) => {
