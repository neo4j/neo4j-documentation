...
class SpatialFunctionsTest extends DocumentingTest {
  ...
  override def doc = new DocBuilder {
    ...
    section("point() - WGS 84 2D", "functions-point-wgs84-2d") {
      ...
      considerations("If any argument provided to `point()` is `null`, `null` will be returned.", "If the coordinates are specified using `latitude` and `longitude`, the `crs` or `srid` fields are optional and inferred to be `'WGS-84'` (srid=4326).", "If the coordinates are specified using `x` and `y`, the `crs` and `srid` fields are optional and inferred to be `'cartesian'` (srid=7203).")
    ...
    section("point() - WGS 84 3D", "functions-point-wgs84-3d") {
      ...
      considerations("If any argument provided to `point()` is `null`, `null` will be returned.", "If the `height/z` key and value is not provided, a 2D point in the _WGS 84_ CRS will be returned.", "If the coordinates are specified using `latitude` and `longitude`, the `crs` or `srid` fields are optional and inferred to be `'WGS-84-3D'` (srid=4979).", "If the coordinates are specified using `x` and `y`, the `crs` and `srid` fields are optional and inferred to be `'cartesian'` (srid=7203).")
