      ...
      query(
        ...
        ResultAssertions((r) => {
          r.toList should equal(
            List(Map("{key: 'Value', listKey: [{inner: 'Map1'}, {inner: 'Map2'}]}" -> Map(
              "key" -> "Value",
              "listKey" -> List(Map("inner" -> "Map1"), Map("inner" -> "Map2"))
            )))
          )
        })
      ) {
    ...
    section("Map projection", "cypher-map-projection") {
      ...
      section("Examples of map projections", "cypher-map-projection-examples") {
          ...
          ResultAssertions((r) => {
            r.toList should equal(
              List(Map("actor" -> Map(
                ...
                "movies" -> List(
                  Map("title" -> "Apocalypse Now", "year" -> 1979),
                  Map("title" -> "Red Dawn", "year" -> 1984),
                  Map("title" -> "Wall Street", "year" -> 1987)
                )
              )))
            )
          })
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toSet should equal(Set(
              Map("actor" -> Map("name" -> "Charlie Sheen", "nbrOfMovies" -> 3)),
              Map("actor" -> Map("name" -> "Martin Sheen", "nbrOfMovies" -> 2))
            ))
          })
        ) {
          resultTable()
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(Map("actor" -> Map(
              "name" -> "Charlie Sheen",
              "realName" -> "Carlos Irwin Estévez",
              "age" -> null
            ))))
