    ...
    section("Introduction", "cypher-spatial-introduction") {
      p(
        """
          ...
          |  There is no special syntax for creating spatial indexes, as it is supported using the existing <<administration-indexes-create-a-single-property-range-index-for-nodes,indexes>>.
      ...
      section("Accessing components of points", "cypher-spatial-accessing-components-spatial-instants"){
        ...
        ResultAssertions((r) => {
            withClue("Expect the components to have the right values") {
              ...
              r.head("x") should be(4)
              r.head("y") should be(3)
              r.head("z") should be(4321)
              r.head("latitude") should be(r.head("y"))
