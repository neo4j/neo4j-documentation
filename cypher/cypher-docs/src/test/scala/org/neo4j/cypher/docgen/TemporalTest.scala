  ...
  override def doc = new DocBuilder {
    ...
    section("Time zones", "cypher-temporal-timezones") {
      ...
      p(
        ...
        """.stripMargin)
      p("""If the rules for a time zone changes in the time zone database after the creation of a temporal instant -- for example, alterations to the rules for daylight savings time in a certain area -- the presented time could differ from the originally-entered time insofar as the local timezone is concerned.
    ...
    section("Specifying durations", "cypher-temporal-specifying-durations") {
      ...
      p("""The following table lists the component identifiers for the unit based form:
          |
          |[options="header", width="85%"]
          ||===
