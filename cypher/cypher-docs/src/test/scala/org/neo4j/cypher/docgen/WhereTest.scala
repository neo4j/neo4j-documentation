        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("name" -> "Andy", "age" -> 36L),
              Map("name" -> "Peter", "age" -> 35L),
              Map("name" -> "Timothy", "age" -> 25L)
            ))
          })
        ) {
          resultTable()
    ...
    section("Using path patterns in `WHERE`", "query-where-patterns") {
      ...
      section("Filter on patterns using `NOT`", "filter-on-patterns-using-not") {
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("person.name" -> "Timothy", "person.age" -> 25L),
              Map("person.name" -> "Peter", "person.age" -> 35L)
            ))
          })
        ) {
          p(
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toSet should equal(Set(
              Map("type(r)" -> "KNOWS", "r.since" -> 1999),
              Map("type(r)" -> "KNOWS", "r.since" -> 2012)
            ))
          })
        ) {
    ...
    section("Lists", "query-where-lists") {
      section("`IN` operator", "where-in-operator") {
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("a.name" -> "Timothy", "a.age" -> 25L),
              Map("a.name" -> "Peter", "a.age" -> 35L)
            ))
          })
        ) {
    ...
    section("Missing properties and values", "missing-properties-and-values") {
      ...
      section("Default to `true` if property is missing", "default-to-true-missing-property") {
        ...
        query(
          ...
          ResultAssertions((r) => {
            r.toList should equal(List(
              Map("n.name" -> "Andy", "n.age" -> 36L, "n.belt" -> "white"),
              Map("n.name" -> "Peter", "n.age" -> 35L, "n.belt" -> null),
              Map("n.name" -> "Timothy", "n.age" -> 25L, "n.belt" -> null)
            ))
          })
        ) {
      ...
      section("Relationship pattern predicates", "relationship-pattern-predicates") {
        ...
        query(
          """WITH 2000 AS minYear
            ...
            #RETURN r.since""".stripMargin('#'),
          ErrorAssertions(_.getMessage should startWith(
