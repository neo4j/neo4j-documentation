...
class SecurityAdministrationTest extends DocumentingTest with QueryStatisticsTestSupport {
  ...
  override def doc: Document = new DocBuilder {
    ...
    section("The `admin` role", "administration-security-administration-introduction", "enterprise-edition") {
      ...
      p("In case the built-in admin role has been altered or dropped and needs to be restored to its original state, you can find a step by step guide on how to do this in " +
        "<<operations-manual#recover-admin-role, Operations Manual -> Password and user recovery>>")
    }
