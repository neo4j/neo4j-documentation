...
class labelExpressionsTest extends DocumentingTest with QueryStatisticsTestSupport {
  ...
  override def doc = new DocBuilder {
    ...
    note{
      p("""In earlier versions of Neo4j, label expressions for nodes had a single colon operator that represented the `AND` operator. With the release of version 5.0, a new label expression with an extended set of logical operators is being introduced, in addition to the single colon operator. For information related to the restrictions on mixing the different versions of the label expression syntax, see below.""".stripMargin('#'))
    }
