...
class SchemaIndexTest extends DocumentingTestBase with QueryStatisticsTestSupport with GraphIcing {
  ...
  @Test def create_index_on_a_label() {
    testQuery(
      ...
      text = "To create an index on a property for all nodes having a certain label, use `CREATE INDEX ON`. " +
  ...
  @Test def drop_index_on_a_label() {
    prepareAndTestQuery(
      title = "Drop an index",
      text = "To drop an index from all nodes that have a label and property combination, use the `DROP INDEX` clause.",
