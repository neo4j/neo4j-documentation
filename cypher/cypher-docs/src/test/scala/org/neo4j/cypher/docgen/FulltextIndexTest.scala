...
class FulltextIndexTest extends DocumentingTest {
  ...
  override def doc: Document = new DocBuilder {
    ...
    private val createMatrixMovieNode =
      """CREATE (m:Movie {title: "The Matrix", reviews: ["The best movie ever", "The movie is nonsense"]}) RETURN m.title"""

    private val createJacketMovies =
    ...
    private val createTitleAndDescriptionFulltextIndex =
    ...
    private val queryForMatrixNode =
    ...
    private val createRelationshipFulltextIndexWithConfig =
    ...
    p("""At first sight, the construction of full-text indexes can seem similar to regular indexes.
        ...
        #In contrast to <<administration-indexes-search-performance, other indexes>>, a full-text index can be:""".stripMargin(
      '#'
    ))
    ...
    section("Query full-text indexes", "administration-indexes-fulltext-search-query") {
      ...
      query(
        """CALL db.index.fulltext.queryNodes("titlesAndDescriptions", "Full Metal Jacket") YIELD node, score
          #RETURN node.title, score""".stripMargin('#'),
        ResultAssertions(r => {
    ...
    section("Handling of Text Array properties", id = "administration-indexes-fulltext-search-text-array-properties") {
      ...
      query(
        """CALL db.index.fulltext.queryNodes("reviews", 'best') YIELD node, score
          #RETURN node.title, node.reviews, score;""".stripMargin('#'),
        ResultAssertions(r => {
          r.size should equal(1)
        })
      ) {
