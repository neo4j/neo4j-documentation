...
class ScalarFunctionsTest extends DocumentingTest {
  ...
  override def doc = new DocBuilder {
    ...
    section("elementId()", "functions-element-id") {
      ...
      query(
        """MATCH (a)
          #RETURN elementId(a)""".stripMargin('#'),
        ResultAssertions((r) => {
    ...
    section("[deprecated]#id()", "functions-id") {
      p(
        """The `id()` function will be removed in the next major release. It is recommended to use <<functions-element-id, `elementId()`>> instead."""
      )
    ...
    section("size() applied to pattern comprehension", "functions-size-of-pattern-comprehension") {
      ...
      function("size(pattern comprehension)", ("pattern comprehension", "A pattern comprehension that returns a list."))
      query(
        """MATCH (a)
          #WHERE a.name = 'Alice'
      ...
      query(
        ...
        ResultAssertions((r) => {
          r.toList should equal(List(Map(
            ...
            "toBooleanOrNull('not a boolean')" -> null,
            "toBooleanOrNull(0)" -> false,
            "toBooleanOrNull(1.5)" -> null
          )))
        })
      ) {
        resultTable()
    ...
    section("toFloatOrNull()", "functions-tofloatornull") {
      ...
      query(
        ...
        ResultAssertions((r) => {
          r.toList should equal(List(Map(
            "toFloatOrNull('11.5')" -> 11.5,
            "toFloatOrNull('not a number')" -> null,
            "toFloatOrNull(true)" -> null
          )))
        })
      ) {
        resultTable()
    ...
    section("toIntegerOrNull()", "functions-tointegerornull") {
      ...
      query(
        ...
        ResultAssertions((r) => {
          r.toList should equal(List(Map(
            "toIntegerOrNull('42')" -> 42,
            "toIntegerOrNull('not a number')" -> null,
            "toIntegerOrNull(true)" -> 1,
            "toIntegerOrNull(['A', 'B', 'C'])" -> null
          )))
        })
      ) {
        resultTable()
