...
class ScalarFunctionsTest extends DocumentingTest {
  ...
  override def doc = new DocBuilder {
    ...
    section("toBoolean()", "functions-toboolean") {
      p(
        """`toBoolean()` converts the argument to a boolean.
          |It attempts to convert a string to a boolean value, and if passed a boolean, it will leave it unchanged.
          |If the parsing fails, `null` will be returned.""".stripMargin)
      function("toBoolean(expression)", ("expression", "An expression that returns a boolean or a string."))
      query(
    ...
    section("toInteger()", "functions-tointeger") {
      ...
      function("toInteger(expression)", ("expression", "An expression that returns anything."))
      query(
