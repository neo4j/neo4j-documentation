...
class UsingTest extends DocumentingTest {
  ...
  case class ShouldUseNodeIndexSeekOn(variable: String) extends PlanAssertion {
    override def matcher: Matcher[String] = include regex s"NodeIndexSeek\\s*\\|\\s*BTREE INDEX $variable".r
  }

  case class ShouldUseRelationshipIndexSeekOn(variable: String) extends PlanAssertion {
    override def matcher: Matcher[String] = include regex s"(Undirected|Directed)RelationshipIndexSeek\\s*\\|\\s*BTREE INDEX \\(\\w*\\)-\\[$variable".r
  }
