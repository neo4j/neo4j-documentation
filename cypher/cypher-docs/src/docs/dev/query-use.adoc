...
The `USE` clause can only appear as the first clause of queries:
...
When executing queries against a Fabric database, the `USE` clause can also appear as the first clause of:
...
When executing queries against a Fabric database, in addition to referring to databases in the DBMS, the `<graph>` may refer to a graph mounted through the Fabric configuration.
