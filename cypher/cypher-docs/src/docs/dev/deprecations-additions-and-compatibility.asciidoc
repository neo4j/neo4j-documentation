...
| `SHOW CONSTRAINTS WHERE ...`                          | Functionality | Updated    | Now allows filtering for `SHOW CONSTRAINTS`
| `SHOW INDEXES YIELD ... [WHERE ...] [RETURN ...]`     | Functionality | Updated    | Now allows `YIELD`, `WHERE` and `RETURN` clauses to `SHOW INDEXES` to change the output.
| `SHOW CONSTRAINTS YIELD ... [WHERE ...] [RETURN ...]` | Functionality | Updated    | Now allows `YIELD`, `WHERE` and `RETURN` clauses to `SHOW CONSTRAINTS` to change the output.
...
| `SHOW [PROPERTY] EXIST[ENCE] CONSTRAINTS`                    | Syntax | Updated    | New syntax for filtering `SHOW CONSTRAINTS` on property existence constraints. Allows `YIELD` and `WHERE` but not `BRIEF` or `VERBOSE`.
| `SHOW NODE [PROPERTY] EXIST[ENCE] CONSTRAINTS`               | Syntax | Updated    | New syntax for filtering `SHOW CONSTRAINTS` on node property existence constraints. Allows `YIELD` and `WHERE` but not `BRIEF` or `VERBOSE`.
| `SHOW REL[ATIONSHIP] [PROPERTY] EXIST[ENCE] CONSTRAINTS`     | Syntax | Updated    | New syntax for filtering `SHOW CONSTRAINTS` on relationship property existence constraints. Allows `YIELD` and `WHERE` but not `BRIEF` or `VERBOSE`.
| `SHOW EXISTS CONSTRAINTS`              | Syntax | Deprecated | Replaced by `SHOW [PROPERTY] EXIST[ENCE] CONSTRAINTS`.  Still allows `BRIEF` and `VERBOSE` but not `YIELD` or `WHERE`.
| `SHOW NODE EXISTS CONSTRAINTS`         | Syntax | Deprecated | Replaced by `SHOW NODE [PROPERTY] EXIST[ENCE] CONSTRAINTS`.  Still allows `BRIEF` and `VERBOSE` but not `YIELD` or `WHERE`.
| `SHOW RELATIONSHIP EXISTS CONSTRAINTS` | Syntax | Deprecated | Replaced by `SHOW RELATIONSHIP [PROPERTY] EXIST[ENCE] CONSTRAINTS`.  Still allows `BRIEF` and `VERBOSE` but not `YIELD` or `WHERE`.
...
| `CREATE [BTREE] INDEX [name] FOR ()-[r:TYPE]-() ON (r.prop) [OPTIONS {...}]`  | Functionality | Added | Allows creating indexes on relationships with a particular relationship type and property combination. They can be dropped by using their name.
| `CREATE LOOKUP INDEX [name] FOR (n) ON EACH labels(n) [OPTIONS {}]`           | Functionality | Added | Create lookup index for nodes with any labels. They can be dropped by using their name.
| `CREATE LOOKUP INDEX [name] FOR ()-[r]-() ON [EACH] type(r) [OPTIONS {}]`     | Functionality | Added | Create lookup index for relationships with any types. They can be dropped by using their name.
...
| `SHOW LOOKUP INDEXES`     | Functionality | Updated  | Now allows easy filtering for `SHOW INDEXES` on lookup indexes. Allows `YIELD` and `WHERE` but not `BRIEF` or `VERBOSE`.
| `CREATE DATABASE ... [OPTIONS {...}]` | Syntax  | Added | New syntax to pass options to `CREATE DATABASE`.  This can be used to specify a specific cluster node to seed data from.
| `RENAME ROLE` | Functionality | Added | New Cypher command for changing the name of a role.
| `RENAME USER` | Functionality | Added | New Cypher command for changing the name of a user.
| `SHOW PROCEDURE[S] [EXECUTABLE [BY {CURRENT USER | username}]] [YIELD ...] [WHERE ...] [RETURN ...]`                                  | Functionality | Added | New Cypher commands for listing procedures.
| `SHOW [ALL \| BUILT IN \| USER DEFINED] FUNCTION[S] [EXECUTABLE [BY {CURRENT USER | username}]] [YIELD ...] [WHERE ...] [RETURN ...]` | Functionality | Added | New Cypher commands for listing functions.
|===
...
| `db.schemaStatements`       | Procedure     | Deprecated | Replaced by `SHOW INDEXES YIELD *` and `SHOW CONSTRAINTS YIELD *`
