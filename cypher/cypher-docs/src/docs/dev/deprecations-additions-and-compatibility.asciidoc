...
----
a|
`shortestPath` and `allShortestPaths` without <<cypher-pattern-varlength, variable-length relationship>> have been deprecated. Instead, use a `MATCH` with a `LIMIT` of 1 or:
[source, cypher, role="noheader"]
----
