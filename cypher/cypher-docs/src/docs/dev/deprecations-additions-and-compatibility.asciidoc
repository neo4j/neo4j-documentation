...
When the command is not in standalone mode, the `YIELD` and `RETURN` are mandatory. 
`YIELD *` is not allowed.

The `transaction-id` is a comma-separated list of one or more quoted strings.
