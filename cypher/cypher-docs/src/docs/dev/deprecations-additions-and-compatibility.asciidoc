...
----
a|
New columns have been added to the result:
...
a|
The matching of null == null in CASE expressions was updated and is no longer able to be matched on.
If matching on null is desired, please use `IS NULL` instead.
[source, cypher, role="noheader"]
----
MATCH (n)
RETURN
