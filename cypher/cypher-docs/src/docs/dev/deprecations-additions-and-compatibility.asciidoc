...
| <<property-types, Temporal types>>  | Functionality | Added | Supports storing, indexing and working with the following temporal types: Date, Time, LocalTime, DateTime, LocalDateTime and <<structural-types-duration, Duration>>.
|===
