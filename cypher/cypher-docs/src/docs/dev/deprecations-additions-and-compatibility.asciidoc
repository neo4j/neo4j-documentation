...
| `CREATE [BTREE] INDEX [name] FOR ()-[r:TYPE]-() ON (r.prop) [OPTIONS {...}]`  | Functionality | Added | Allows creating indexes on relationships.
| `CREATE LOOKUP INDEX [name] FOR (n) ON EACH labels(n) [OPTIONS {}]`           | Functionality | Added | Create lookup index for nodes with any labels.
| `CREATE LOOKUP INDEX [name] FOR ()-[r]-() ON [EACH] type(r) [OPTIONS {}]`     | Functionality | Added | Create lookup index for relationships with any types.
