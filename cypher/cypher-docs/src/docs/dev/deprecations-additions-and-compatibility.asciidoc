...
| <<administration-indexes-introduction, Composite indexes>> | Index | Added |
...
In Neo4j 4.0 this has changed again, such that the compatibility layer includes only the parser.
...
WARNING: In Neo4j 4.0 some older language features are understood by the Cypher parser even if they are no longer supported by the Neo4j kernel.
