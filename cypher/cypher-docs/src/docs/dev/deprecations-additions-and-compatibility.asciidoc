...
| <<java-reference#extending-neo4j-cypher-functions, User-defined functions>> | Functionality | Added |
...
| `MATCH (n)-[x:A\|:B\|:C*]-() RETURN n`     | Syntax | Deprecated | Replaced by `MATCH (n)-[x:A\|B\|C*]-() RETURN n`
| <<java-reference#extending-neo4j-cypher-aggregation-functions, User-defined aggregation functions>> | Functionality | Added |
