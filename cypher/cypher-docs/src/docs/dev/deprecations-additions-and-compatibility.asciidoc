...
----
CREATE CONSTRAINT [name] ON (node:Label)
...
| `CREATE INDEX FOR ()-[r:TYPE]-() ...`         | Functionality | Added         | Allows creating indexes on relationships with a particular relationship type and property combination. They can be dropped by using their name.
| `CREATE LOOKUP INDEX ...`                     | Functionality | Added         | Create token lookup index for nodes with any labels or relationships with any relationship type. They can be dropped by using their name.
...
| `SHOW LOOKUP INDEXES`     | Functionality | Updated  | Now allows easy filtering for `SHOW INDEXES` on token lookup indexes. Allows `YIELD` and `WHERE` but not `BRIEF` or `VERBOSE`.
...
| `CREATE FULLTEXT INDEX ...`   | Syntax | Added | Allows creating fulltext indexes on nodes or relationships. They can be dropped by using their name.
|===
