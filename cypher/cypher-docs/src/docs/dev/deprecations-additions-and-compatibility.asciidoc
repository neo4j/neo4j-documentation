...
----
a|
The `transaction-id` now allows general expressions resolving to a string or a list of strings instead of just parameters.
...
The order of the commands does not matter and there can be more than one of each command.

When the command is not in standalone mode, the `YIELD` and `RETURN` are mandatory and `YIELD *` is not allowed.

The `transaction-id` is a comma-separated list of one or more quoted strings or an expression resolving to a string or a list of strings (for example the output column from `SHOW`).
