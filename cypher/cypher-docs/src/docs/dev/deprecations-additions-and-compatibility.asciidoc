...
| `DROP INDEX name` | Syntax | Added | <<query-plan-drop-index-by-name,New command>> for dropping an index by name.
| `DROP CONSTRAINT name` | Syntax | Added | <<query-plan-drop-constraint-by-name,New command>> for dropping a constraint by name, no matter the type.
