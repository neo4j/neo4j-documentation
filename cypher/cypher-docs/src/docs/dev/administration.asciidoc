...
Most administrative commands must be run against the `system` database because they involve editing the metadata for the entire system.
These are automatically routed to the `system` database when connected to the DBMS over bolt.
