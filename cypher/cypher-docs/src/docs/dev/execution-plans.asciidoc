...
| <<query-plan-drop-index-deprecated-syntax, DropIndex>> (deprecated syntax) | Drops an index from a property for all nodes having a certain label.  | Y | Y |
| <<query-plan-drop-index-new-syntax, DropIndex>> (new syntax) | Drops an index using its name.  | Y | Y |
| <<query-plan-drop-named-constraint, DropConstraint>>     |  Drops a constraint using its name.  | Y | Y |
| <<query-plan-drop-node-key-constraint-deprecated, DropNodeKeyConstraint>>     |  Drops a Node Key from a set of properties for all nodes having a certain label.  | Y | Y |
| <<query-plan-drop-node-property-existence-constraint-deprecated, DropNodePropertyExistenceConstraint>>     | Drops an existence constraint from a property for all nodes having a certain label.  | Y | Y |
| <<query-plan-drop-relationship-property-existence-constraint-deprecated, DropRelationshipPropertyExistenceConstraint>>     | Drops an existence constraint from a property for all relationships of a certain type.  | Y | Y |
| <<query-plan-drop-result, DropResult>>     | Produces zero rows when an expression is guaranteed to produce an empty result.  |  |  |
| <<query-plan-drop-unique-constraint-deprecated, DropUniqueConstraint>>                  | Drops a unique constraint from a property for all nodes having a certain label.  | Y | Y |
...
Note that the presented value can vary slightly depending on the Cypher runtime that was used to execute the query.
