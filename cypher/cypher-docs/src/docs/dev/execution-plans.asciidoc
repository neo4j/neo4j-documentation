...
| <<query-plan-node-unique-index-range-seek, NodeUniqueIndexSeekByRange>>                 | Starting point | Finds nodes using an index seek within a unique index where the value of the property matches the given prefix string.
...
include::execution-plan-groups/combining-operators.asciidoc[leveloffset=+1]
