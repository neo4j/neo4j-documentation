...
* <<execution-plans-result-building-operators, Result-building operators>>
* <<execution-plans-combining-operators, Combining operators>>
...
| <<query-plan-argument, Argument>>                                | Starting point | Indicates the variable to be used as an argument to the right-hand side of an `Apply` operator xxx.
| <<query-plan-assert-same-node, AssertSameNode>>                            | Combining | Used to ensure that no uniqueness constraints are violated.
| <<query-plan-cartesian-product, CartesianProduct>>                 | Combining | xx.
...
| <<query-plan-create-index, CreateIndex>>              | Update | Creates an index xx.
| <<query-plan-create-unique-constraint, CreateUniqueConstraint>>                  | Update | Creates a unique constraint on a (label,property) pair.
| <<query-plan-create-node, CreateNode>>              | Update | Creates a node.
| <<query-plan-create-node-property-existence-constraint, CreateNodePropertyExistenceConstraint>>     | Update | Creates an existence constraint on a node property.
| <<query-plan-create-relationship, CreateRelationship>>              | Update | Creates a relationship.
| <<query-plan-create-relationship-property-existence-constraint, CreateRelationshipPropertyExistenceConstraint>>     | Update | Creates an existence constraint on a relationship property.
| <<query-plan-delete, Delete>>                                          | Update |Deletes a node or relationship.
| <<query-plan-detach-delete, DetachDelete>>               | Update |Used when executing a query containing `DETACH DELETE` to delete nodes and their relationships.
...
| <<query-plan-drop-index, DropIndex>>              | Update | Drops an index xx.
| <<query-plan-drop-node-property-existence-constraint, DropNodePropertyExistenceConstraint>>     | Update | Drops an existence constraint from a node property.
| <<query-plan-drop-relationship-property-existence-constraint, DropRelationshipPropertyExistenceConstraint>>     | Update | Drops an existence constraint from a relationship property.
| <<query-plan-drop-unique-constraint, DropUniqueConstraint>>                  | Update | Drops a unique constraint on a (label,property) pair.
...
| <<query-plan-empty-result, EmptyResult>>                                   | Result building | Eagerly loads everything coming in to the EmptyResult operator and discards it.
...
| <<query-plan-foreach, Foreach>>                                              | Combining | xxx
...
| <<query-plan-let-select-or-semi-apply, LetSelectOrSemiApply>>              | Combining | xxxx
| <<query-plan-let-select-or-anti-semi-apply, LetSelectOrAntiSemiApply>>     | Combining | xxx
...
| <<query-plan-load-csv, LoadCSV>>                                                  | Starting point | Used when executing a query containing LOAD CSV.
...
| <<query-plan-merge-create-node, MergeCreateNode>>            | Updating | xxxx.
| <<query-plan-merge-create-relationship, MergeCreateRelationship>>            | Updating | xxxx.
...
| <<query-plan-node-index-contains-scan, NodeIndexContainsScan>>             | Starting point | Goes through all values stored in an index, and searches for entries containing a specific string.
| <<query-plan-node-index-ends-with-scan, NodeIndexEndsWithScan>>        | Starting point | Goes through all values stored in an index, and searches for entries ending in a specific string.
...
| <<query-plan-node-index-seek-by-range, NodeIndexSeekByRange>>                 | Starting point | Finds nodes using an index seek where the value of the property matches the given prefix string.
| <<query-plan-node-outer-hash-join, NodeOuterHashJoin>>      | Combining | xxx.
| <<query-plan-node-unique-index-seek, NodeUniqueIndexSeek>>                        | Starting point | Finds nodes using an index seek within a unique index.
| <<query-plan-node-unique-index-seek-by-range, NodeUniqueIndexSeekByRange>>                 | Starting point | Finds nodes using an index seek within a unique index where the value of the property matches the given prefix string.
...
| <<query-plan-optional-expand-into, OptionalExpand(Into)>>                    | Expand | xx.
| <<query-plan-procedure-call, ProcedureCall>>                               | Row | Calls a procedure.
| <<query-plan-produce-result, ProduceResult>>                                   | Result building | xx.
...
| <<query-plan-remove-labels, RemoveLabels>>   | Update | Used when deleting labels from a node.
| <<query-plan-roll-up-apply, RollUpApply>>      | Combining | xxx
...
| <<query-plan-set-labels, SetLabels>>                            | Update | Used when setting labels on a node.
| <<query-plan-set-node-property, SetNodeProperty>>          | Update | Used when setting a property on a node.
| <<query-plan-set-node-property-from-map, SetNodePropertyFromMap>>          | Update | Used when setting properties from a map on a node.
| <<query-plan-set-relationship-property, SetRelationshipProperty>>          | Update | Used when setting a property on a relationship.
| <<query-plan-set-relationship-property-from-map, SetRelationshipPropertyFromMap>>    | Update | Used when setting properties from a map on a relationship.
...
| <<query-plan-triadic, Triadic>>                                            | Combining | Used to solve triangular queries, such as the very common 'find my friend-of-friends that are not already my friend'.
...
| <<query-plan-value-hash-join, ValueHashJoin>>                       | Combining | xxx.
| <<query-plan-varlength-expand-all, VarLengthExpand(All)>>                                   | Expand | xxx.
| <<query-plan-varlength-expand-into, VarLengthExpand(Into)>>                                   | Expand | xxx.
|===
...
include::execution-plan-groups/result-building.asciidoc[leveloffset=+1]

include::execution-plan-groups/combining.asciidoc[leveloffset=+1]
