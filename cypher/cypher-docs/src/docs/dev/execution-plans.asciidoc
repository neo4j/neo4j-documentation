...
To produce an efficient plan for a query, Neo4j requires information about the database, such as which indexes and constraints are present [add link to the Schema section here].
...
More information about how the statistics are kept up to date, as well as configuration options about managing query replanning can be found in the <<operations-manual#schema-statistics, Operations Manual -> Schema statistics>>
