...
[abstract]
--
This section describes the characteristics of a query's execution plan, along with details about each of the operators.
--

* <<execution-plan-introduction, Introduction>>
* <<execution-plan-operators-summary, Execution plan operators>>
* <<execution-plans-dbhits, DbHits (Database hits)>>
...
The operators are combined into a tree-like structure called an _execution plan_, in which each operator is represented as a node in the tree.
Each operator takes as input zero or more rows, and produces as output zero or more rows -- the output from one operator thus forms the input for the next operator.
Operators that join two branches will effectively combine input from two incoming streams and produce a single output.

.Evaluation model
Evaluation of the execution plan begins at the _leaf_ nodes of the tree.
Leaf nodes have no input rows and generally comprise operators such as scans and seeks which obtain the data directly from the storage engine, thus incurring <<execution-plans-dbhits, database hits>>.
Any rows produced by leaf nodes are then piped into their parent nodes, which in turn pipe their rows to their parent nodes and so on, all the way up to the root node, which produces the final results of the query.

[[eagerness-laziness]]
.Eager and lazy evaluation
In general, query evaluation is _lazy_: most operators start piping their rows as soon as they are produced to their parent operators.
...
However, some operators, such as those used for aggregation and sorting, need to accumulate _all_ their rows before piping any of them to their parent operator(s); i.e. such an operator needs to complete execution in its entirety before _any_ rows are sent to its parent(s) as input.
...
`EstimatedRows`::
This is the estimated number of rows -- i.e. the approximate number of rows based on the available statistical information -- that is expected to be produced by the operator.
...
To produce an efficient plan for a query, the Cypher query planner requires information about the Neo4j database, such as which indexes and constraints are present (see <<cypher-schema>>).
Statistical information maintained by the database is also used to generate an efficient execution plan; i.e. the Cypher query planner uses this information to determine which access patterns will produce the best execution plan.
...
More information about how the statistics are kept up to date, as well as configuration options about managing query replanning and caching can be found in the <<operations-manual#statistics-execution-plans, Operations Manual -> Statistics and execution plans>>

<<query-tuning>> describes how to tune Cypher queries.
In particular, see <<how-do-i-profile-a-query>> for how to view the execution plan for a query and <<query-using>> for how to use planner _hints_ to influence the decisions of the planner when building an execution plan for a query.

For a deeper understanding of how each operator works, refer to <<execution-plan-operators-summary>>.
...
[options="header"]
|===
|Name                                                                       | Description | Leaf? | Updating? | Eager?
...
| <<query-plan-distinct, Distinct>>                                          | Drops duplicate rows from the incoming stream of rows.  | | | Y
...
| <<query-plan-eager, Eager>>                                                | For isolation purposes, `Eager` ensures that operations affecting subsequent operations are executed fully for the whole dataset before continuing execution.  | | | Y
| <<query-plan-eager-aggregation, EagerAggregation>>                         | Evaluates a grouping expression. | | | Y
...
| <<query-plan-node-hash-join, NodeHashJoin>>                                | Executes a hash join on node ids. | | | Y
...
| <<query-plan-sort, Sort>>                                                  | Sorts rows by a provided key. | | | Y
| <<query-plan-top, Top>>                                                    | Returns the first 'n' rows sorted by a provided key. | | | Y
...
| <<query-plan-value-hash-join, ValueHashJoin>>                       | Executes a hash join on arbitrary values. | | | Y
