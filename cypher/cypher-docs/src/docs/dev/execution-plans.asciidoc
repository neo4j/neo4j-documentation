...
The output from one operator becomes the input for the next operator.
Operators that join two branches in the tree combines input from two incoming streams and produce a single output.
