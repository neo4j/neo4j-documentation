...
It may still throw an error should there exist conflicting data, indexes, or constraints.
...
while dropping a constraint require <<administration-security-administration-database-constraints, the `DROP CONSTRAINT` privilege>> and
listing constraints require <<administration-security-administration-database-constraints, the `SHOW CONSTRAINT` privilege>>.
