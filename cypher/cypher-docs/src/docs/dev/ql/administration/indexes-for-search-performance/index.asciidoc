...
There are multiple index types available: b-tree, fulltext, lookup, range, and text index types.
...
.10+.^| Best practice is to give the index a name when it is created.
If the index is not explicitly named, it will get an auto-generated name.
...
| [source, cypher, role=noplay]
----
SHOW [ALL\|BTREE\|FULLTEXT\|LOOKUP\|RANGE\|TEXT] INDEX[ES]
...
while dropping an index requires <<administration-security-administration-database-indexes, the `DROP INDEX` privilege>> and
listing indexes require <<administration-security-administration-database-indexes, the `SHOW INDEX` privilege>>.
...
include::../indexes/create-a-single-property-b-tree-index-for-nodes.asciidoc[leveloffset=+1]

include::../indexes/create-a-single-property-b-tree-index-for-relationships.asciidoc[leveloffset=+1]

include::../indexes/create-a-single-property-b-tree-index-only-if-it-does-not-already-exist.asciidoc[leveloffset=+1]

include::../indexes/create-a-single-property-b-tree-index-with-specified-index-provider.asciidoc[leveloffset=+1]

include::../indexes/create-a-single-property-b-tree-index-with-specified-index-configuration.asciidoc[leveloffset=+1]

include::../indexes/create-a-composite-b-tree-index-for-nodes.asciidoc[leveloffset=+1]

include::../indexes/create-a-composite-b-tree-index-for-relationships.asciidoc[leveloffset=+1]

include::../indexes/create-a-composite-b-tree-index-with-specified-index-provider-and-configuration.asciidoc[leveloffset=+1]
...
| `type`
| The IndexType of this index (`BTREE`, `FULLTEXT`, `LOOKUP`, `RANGE`, or `TEXT`).
| `+`
| `+`
...
[NOTE]
====
While the command for listing indexes require  the <<administration-security-administration-database-indexes, `SHOW INDEX` privilege>>,
...
[NOTE]
====
This syntax only supports dropping node property indexes, all others can only be dropped by name.
====
