...
[NOTE]
====
While the command for listing indexes require <<administration-security-administration-database-indexes, the `SHOW INDEX` privilege>>,
...
[NOTE]
====
This syntax only support dropping node property indexes, all others can only be dropped by name.
====
