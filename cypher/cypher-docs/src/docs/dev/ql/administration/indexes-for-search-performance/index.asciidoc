...
Adding `OR REPLACE` to the command will result in any existing index with that name being deleted and a new one created.
...
include::../indexes/replace-a-composite-index.asciidoc[leveloffset=+1]
