...
[NOTE]
The old deprecated built-in procedures for listing indexes, such as `db.indexes`, work as before and are not affected by the
<<administration-security-administration-database-indexes, `SHOW INDEXES` privilege>>.

include::../indexes/example-for-listing-indexes.asciidoc[leveloffset=+2]
