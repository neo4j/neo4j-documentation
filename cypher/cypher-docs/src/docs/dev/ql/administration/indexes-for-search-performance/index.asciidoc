...
There are multiple index types available: b-tree, fulltext, lookup, point, range, and text index types.
...
.12+.^| Best practice is to give the index a name when it is created.
If the index is not explicitly named, it will get an auto-generated name.
...
| `type`
| The IndexType of this index (`BTREE`, `FULLTEXT`, `LOOKUP`, `POINT`, `RANGE`, or `TEXT`).
| `+`
| `+`
