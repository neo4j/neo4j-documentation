...
It may still throw an error should there exist conflicting data or constraints.
...
while dropping an index require <<administration-security-administration-database-indexes, the `DROP INDEX` privilege>> and
