...
|<<functions-all,all()>>          | Tests whether a predicate holds for all elements in a given list.
|<<functions-any,any()>>          | Tests whether a predicate holds for at least one element in a given list.
|<<functions-exists,exists()>>    | Returns true if a match for a given pattern exists in the graph, or if a specified property exists in the node, relationship or map.
|<<functions-none,none()>>        | Returns true if the predicate holds for no element in a given list.
|<<functions-single,single()>>    | Returns true if the predicate holds for exactly one of the elements in a given list.
|===
...
|===
| Function                                                            | Description
| <<functions-coalesce,coalesce()>>                                   | Returns the first non-`null` value in a given list of expressions.
...
| <<functions-toboolean,toBoolean()>>                                 | Casts a string value to a boolean value.
| <<functions-tofloat,toFloat()>>                                     | Casts an integer or string value to a floating point number.
| <<functions-tointeger,toInteger()>>                                 | Casts a floating point or string value to an integer value.
...
|<<functions-collect,collect()>>                | Returns a list containing the values returned by a given expression.
...
|<<functions-sum,sum()>>                        | Returns the sum of a set of non-`null` numeric values.
|===
...
|<<functions-filter,filter()>>                  | Returns a list `l~result~` containing all the elements from a list `list` that comply with a given predicate.
...
|<<functions-range,range()>>                    | Returns a list comprising all integral values within a range bounded by a start and end value, where the difference between any two consecutive values is constant.
|<<functions-reduce,reduce()>>                  | Returns the value resulting from the application of an expression on each successive element in a list in conjunction with the result of the computation thus far.
|<<functions-relationships,relationships()/rels()>>    | Returns a list containing all the relationships in a path.
|<<functions-tail,tail()>>                      | Returns a list `l~result~` containing all the elements, excluding the first one, from a list `list`.
|===
...
|===
| Function                     | Description
| <<functions-abs, abs()>>     | Returns the absolute value of a given number.
| <<functions-ceil, ceil()>>   | Returns the smallest floating point number greater than or equal to a given number, and which is equal to an integer.
| <<functions-floor, floor()>> | Returns the largest floating point number less than or equal to a given number, and which is equal to an integer.
| <<functions-rand, rand()>>   | Returns a random floating point number in the range from 0 (inclusive) to 1 (exclusive); i.e. [0,1).
| <<functions-round, round()>> | Returns the value of a given number rounded to the nearest integer.
| <<functions-sign, sign()>>   | Returns the signum of a given number: `0` if the number is `0`, `-1` for any negative number, and `1` for any positive number.
|===
...
|<<functions-replace,replace()>>        | Returns a string in which all occurrences of a specified string in the original string have been replaced by another string.
|<<functions-reverse,reverse()>>        | Returns a string in which all characters in the original string have been reversed.
...
|<<functions-split,split()>>            | Returns a list of strings resulting from the splitting of the original string around matches of a given delimiter.
...
|<<functions-tostring,toString()>>      | Casts an integer, float or boolean value to a string.
...
|===
|Function                               |   Description
|<<functions-distance,distance()>>      | Returns a floating point number representing the geodesic distance between any two given points in the same CRS.
...
For developing and deploy user-defined functions in Neo4j, see <<user-defined-functions, Extending Neo4j -> User-defined functions>>.
