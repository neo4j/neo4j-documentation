...
|<<functions-all,all()>>          | Tests whether a predicate holds for all elements in a list.
|<<functions-any,any()>>          | Tests whether a predicate holds for at least one element in a list.
|<<functions-exists,exists()>>    | Returns true if a match for a pattern exists in the graph, or if a specified property exists in the node, relationship or map.
...
| <<functions-ceil, ceil()>>   | Returns the smallest floating point number greater than or equal to a number.
| <<functions-floor, floor()>> | Returns the largest floating point number less than or equal to a number.
