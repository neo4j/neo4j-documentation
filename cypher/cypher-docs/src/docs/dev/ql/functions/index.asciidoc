...
[NOTE]
Functions taking a string as input all operate on _Unicode code points_ rather than on a standard `char[]`.
...
| <<functions-size-of-string,size() applied to string>>                         | Returns the number of Unicode code points in a string.
