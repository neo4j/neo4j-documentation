...
* <<schema-index-range-comparisons-using-where-single-property-index, Use index with `WHERE` using range comparisons>>
...
include::use-a-single-property-index-with-where-containing-one-or-more-equality-predicates.asciidoc[]
