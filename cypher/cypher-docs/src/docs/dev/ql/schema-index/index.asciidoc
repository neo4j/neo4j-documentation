...
[abstract]
--
This section explains how to configure indexes to enhance performance in search, and to support fulltext search.
--
...
However, predicates might be rewritten into existence check and a filter.
The restrictions for what the index can handle without rewrites are, in order:

* `0-* equality check and list membership check predicates`
* `0-1 range or prefix search predicate`
* `0-* existence check predicates`

Predicates not following the order above will be rewritten to existence check and filter.
For example, an index on `:Label(prop1,prop2,prop3,prop4,prop5,prop6)` and predicates:

```
equality on prop1, equality on prop2, range on prop3, range on prop4, equality on prop5, exists on prop6
```

will be rewritten as:

```
equality on prop1, equality on prop2, range on prop3, exists on prop4, exists on prop5, exists on prop6
```

with filters on `prop4` and `prop5` for the range and equality predicates.
