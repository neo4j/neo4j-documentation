...
* `LOAD CSV` is often used in conjunction with the query clause `CALL +++{ ... }+++ TRANSACTIONS`. For more information, see <<subquery-call-in-transactions, CALL +++{ ... }+++ IN TRANSACTIONS>>.
...
<<operations-manual#config_server.directories.import, server.directories.import>>::
