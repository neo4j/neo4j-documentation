...
Each Cypher query gets optimized and transformed into an <<execution-plan-introduction, execution plan>> by the Cypher execution engine.
...
Each query is turned into an <<execution-plan-introduction, execution plan>> by the _execution planner_.
