...
| `runtime=interpreted` | This will force the query planner to use the interpreted runtime.
...
| `runtime=slotted`     | This will cause the query planner to use the slotted runtime.
                        | [enterprise-edition]#This is the default option for all queries which are not supported by `runtime=pipelined` in Enterprise Edition.#
| `runtime=pipelined`   | This will cause the query planner to use the pipelined runtime if it supports the query.
                          If the pipelined runtime does not support the query, the planner will fall back to the slotted runtime.
                        | [enterprise-edition]#This is the default option for some queries in Enterprise Edition.#
|===
