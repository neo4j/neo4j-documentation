...
|===
| Query option | Description | Default
| `planner=cost` | Use cost based planning with default limits on plan search space and time. | X
...
|===
| Query option | Description | Default
| `connectComponentsPlanner=greedy` | Use a greedy approach when combining sub-plans. | X
...
=== Cypher update strategy
This option affects eagerness of updating queries
[options="header"]
|===
| Query option | Description | Default
| `updateStrategy=default` | Update queries are executed eagerly when needed. | X
| `updateStrategy=eager`   | Update queries are always executed eagerly. |
|===

[[cypher-expression-engine]]
=== Cypher expression engine
This option affects how the runtime evaluates expressions. The possible values are:
[options="header"]
|===
| Query option | Description | Default
| `expressionEngine=default`     | Compile expressions and use the compiled expression engine when needed. | X
| `expressionEngine=interpreted` | Always use the interpreted expression engine. |
| `expressionEngine=compiled`    | Always compile expressions and use the compiled expression engine. Cannot be used together with `runtime=interpreted` |
|===

[[cypher-operator-engine]]
=== Cypher operator engine
This query option affects whether the pipelined runtime attempts to fuse groups of operators into pipelines. The possible values are:
[options="header"]
|===
| Query option | Description | Default
| `operatorEngine=default`     | Attempt to fuse operators when applicable. | X
| `operatorEngine=interpreted` | Never attempt to fuse operators. |
| `operatorEngine=compiled`    | Always attempt to fuse operators. Cannot be used together with `runtime=interpreted` or `runtime=slotted` |
|===

[[cypher-interpreted-pipes-fallback]]
=== Cypher interpreted pipes fallback
This query option affects how the pipelined runtime behaves for operators it does not directly support. The available options are:
[options="header"]
|===
| Query option | Description | Default
...
| Parts of the execution plan may be executed on another runtime.
  ...
  This setting is experimental and using it is discouraged.
