...
In Neo4j 5.0, the community interpreted runtime has been updated to use the slotted runtime, which used to be an enterprise-only feature.
The slotted runtime works similarly, except for some additional optimizations regarding how the records are streamed through the iterators.
