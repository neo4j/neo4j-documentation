...
=== Cypher operator engine
This query option affects whether the pipelined runtime attempts to generate compiled code for groups of operators. The possible values are:
[options="header"]
|===
| Query option | Description | Default
| `operatorEngine=default`     | Attempt to generate compiled operators when applicable. | X
| `operatorEngine=interpreted` | Never attempt to generate compiled operators. |
| `operatorEngine=compiled`    | Always attempt to generate compiled operators. Cannot be used together with `runtime=interpreted` or `runtime=slotted` |
|===
