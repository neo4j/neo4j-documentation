...
The tree comprises nested iterators, which stream records in a pipelined manner starting from the top iterator, which pulls from the next iterator, and so on.
...
In the _pipelined_ runtime, the operators are grouped in the execution plan to generate new combinations and orders of execution, which are optimized for performance and memory usage. +
