...
Each query is turned into an <<execution-plan-introduction, execution plan>> by the Cypher _query planner_.
...
[NOTE]
In Enterprise Edition, the Cypher query planner's fallback mechanism is to try the compiled runtime first, and if it does not support the query, to then try the slotted runtime, and if that also does not support the query, to finally fall back to the interpreted runtime, which supports all queries.
