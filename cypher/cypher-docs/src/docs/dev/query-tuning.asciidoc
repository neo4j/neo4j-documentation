...
* The time limit has past the <<operations-manual#config_cypher.min_replan_interval,cypher.min_replan_interval>> value, and the database statistics have changed more than the <<operations-manual#config_cypher.statistics_divergence_threshold,cypher.statistics_divergence_threshold>> value.

[NOTE]
Replanning is not performed for all queries at once, however it is performed in the same thread as running the query, and can block the query.
