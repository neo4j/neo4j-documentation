...
* <<query-plan-all-nodes-scan>> - Reads all nodes from the node store.
* <<query-plan-directed-relationship-by-id-seek, DirectedRelationshipByIdSeek>> - Reads one or more relationships by id from the relationship store.
* <<query-plan-node-by-id-seek, NodeByIdSeek>> - Reads one or more nodes by id from the node store.
