...
//WHERE EXISTS { ... }
m|<<query-exists, WHERE EXISTS +++{ ... }+++>>
|An existential sub-query used to filter the results of a `MATCH`, `OPTIONAL MATCH` or `WITH` clause.

// COUNT { ... }
m|<<query-count, COUNT +++{ ... }+++>>
|An expression used to compute the number of results of a subquery inside another clause, e.g. `MATCH`, `WITH`, `SET`, etc.
...
include::ql/query-exists.adoc[leveloffset=+1]

include::ql/query-count.adoc[leveloffset=+1]
