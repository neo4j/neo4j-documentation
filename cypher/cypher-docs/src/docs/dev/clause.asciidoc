...
|<<administration-indexes-syntax,CREATE \| DROP INDEX>>                | Create or drop an index on all nodes with a particular label and property.
|<<administration-constraints-syntax,CREATE \| DROP CONSTRAINT>>       | Create or drop a constraint pertaining to either a node label or relationship type, and a property.
