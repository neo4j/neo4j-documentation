...
|<<query-call,CALL {} (subquery)>>                  | Evaluates a subquery.
|===
...
|<<administration-indexes,CREATE \| DROP INDEX>>                       | Create or drop a constraint pertaining to either a node label or relationship type, and a property.
|<<administration-constraints,CREATE \| DROP CONSTRAINT>>              | Create or drop an index on all nodes with a particular label and property.
