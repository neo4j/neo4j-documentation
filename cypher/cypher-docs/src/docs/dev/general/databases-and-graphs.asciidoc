...
For more information about the _system_ database, see the sections on <<databases, Database management>> and <<access-control, Access Control>>.
...
| Role-based security | <<administration-security, User, Role and Privilege management>> for flexible access control and <<access-control-manage-privileges, sub-graph access control>>. | <<access-control-manage-users, Multi-user management>>. All users have full access rights.
