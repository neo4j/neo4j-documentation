...
Cypher is simple but powerful:
Very complicated database queries can easily be expressed, which allows you to focus on your domain instead of getting lost in database access.

Cypher is inspired by a number of different approaches and builds on established practices for expressive querying.
Many of the keywords, for example `WHERE` and `ORDER BY`, are inspired by http://en.wikipedia.org/wiki/SQL[SQL].
