...
That means that `POINT` indexes are only used when it is known that the predicate evaluates to `null` for all non-point values.
...
* `POINT` indexes are used over `RANGE` and `TEXT` indexes for distance and within bounding box.
