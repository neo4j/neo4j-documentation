...
An important exception to this is `TOKEN LOOKUP` indexes, which solve only label predicates.
...
The lookup indexes are the most important indexes in the database and greatly improve the performance of Cypher queries, as well as the population of other indexes.
Dropping these indexes may lead to severe performance degradations.
