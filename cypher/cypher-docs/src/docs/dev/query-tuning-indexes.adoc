...
In some cases it is not possible for the system to determine an expression to be of type string.
...
In such cases the author of the query can modify the query to provide this information. Depending on how values which are not of type string should be treated, there are two options:
