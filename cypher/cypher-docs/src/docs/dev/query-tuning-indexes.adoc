...
There are different types of indexes available in Neo4j.
Not all indexes are compatible with the same property predicates.

In the most common case, indexes are used for `MATCH` and `OPTIONAL MATCH` clauses that combine a label predicate with a property predicate.
Therefore, it is important to know the kind of predicates that can be solved by the different indexes.
...
That means that `TEXT` indexes will only be used when it is known that the predicate will evaluate to `null` for all non-string values.
...
Other predicates, however, will only be used when it is known that the property is compared to a string:

* `n.prop = "string"`
* `n.prop IN ["a", "b", "c"]`
* `n.prop > "string"`

That means that a `TEXT` index will not be able to solve e.g. `a.prop = b.prop`.
...
When multiple indexes are available and able to solve a predicate there is an ordering defined that decide on which index to use.
It is defined such that:

* `TEXT` indexes will be preferred over `BTREE` indexes.
...
include::ql/administration/indexes/when-multiple-index-types-are-available.asciidoc[leveloffset=2]
