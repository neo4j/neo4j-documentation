 ...
 ** <<syntax-no-label, Match on single node label>>
...
=== List of Label Expressions examples and what they match
In the following table, a green tick is shown if the `Label Expression` would match the node.
...
=== Limitations
Label expressions cannot be combined with label syntax eg. `:A:B&C` this will instead throw an error. Instead user either `:A&B&C` or `:A:B:C`.
