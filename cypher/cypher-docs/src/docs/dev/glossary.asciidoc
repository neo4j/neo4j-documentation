...
|<<administration-constraints-create-a-named-node-property-existence-constraint, CREATE CONSTRAINT existence ON (n:Label) ASSERT exists(n.property)>>  | Schema   | Create a named constraint ensuring that all nodes with a particular label have a certain property.
|<<administration-constraints-create-a-named-node-key-constraint, CREATE CONSTRAINT node_key ON (n:Label) ASSERT (n.prop1, ..., n.propN) IS NODE KEY>>  |  Schema | Create a named constraint ensuring all nodes with a particular label have all the specified properties and that the combination of property values is unique; i.e. ensures existence and uniqueness.
|<<administration-constraints-create-a-named-relationship-property-existence-constraint, CREATE CONSTRAINT existence ON ()-[r:REL_TYPE]-() ASSERT exists(r.property)>>  | Schema   | Create a named constraint ensuring that all relationship with a particular type have a certain property.
|<<administration-constraints-create-a-named-unique-constraint, CREATE CONSTRAINT uniqueness ON (n:Label) ASSERT n.property IS UNIQUE>>  |  Schema | Create a named constraint ensuring the uniqueness of the combination of node label and property value for a particular property key across all nodes.
|<<administration-indexes-create-a-named-single-property-index, CREATE INDEX single FOR (n:Label) ON (n.property)>>  | Schema  | Create a named index on all nodes with a particular label and a single property; i.e. create a single-property index.
|<<administration-indexes-create-a-named-composite-index, CREATE INDEX composite FOR (n:Label) ON (n.prop1, ..., n.propN)>>  | Schema  | Create a named index on all nodes with a particular label and multiple properties; i.e. create a composite index.
...
|<<administration-constraints-drop-a-named-constraint, DROP CONSTRAINT name>>      | Schema   | Drop a constraint using the name.
|<<administration-indexes-drop-a-named-index, DROP INDEX name>>       | Schema | Drop an index using the name.
