...
** <<glossary-privileges, Privileges>>
...
|<<administration-constraints-create-a-named-node-property-existence-constraint, CREATE CONSTRAINT [existence] ON (n:Label) ASSERT exists(n.property)>>  | Schema   | Create a constraint ensuring that all nodes with a particular label have a certain property.
|<<administration-constraints-create-a-named-node-key-constraint, CREATE CONSTRAINT [node_key] ON (n:Label) ASSERT (n.prop1, ..., n.propN) IS NODE KEY>>  |  Schema | Create a constraint ensuring all nodes with a particular label have all the specified properties and that the combination of property values is unique; i.e. ensures existence and uniqueness.
|<<administration-constraints-create-a-named-relationship-property-existence-constraint, CREATE CONSTRAINT [existence] ON ()-[r:REL_TYPE]-() ASSERT exists(r.property)>>  | Schema   | Create a constraint ensuring that all relationship with a particular type have a certain property.
|<<administration-constraints-create-a-named-unique-constraint, CREATE CONSTRAINT [uniqueness] ON (n:Label) ASSERT n.property IS UNIQUE>>  |  Schema | Create a constraint ensuring the uniqueness of the combination of node label and property value for a particular property key across all nodes.
|<<administration-indexes-create-a-named-single-property-index, CREATE INDEX [single] FOR (n:Label) ON (n.property)>>  | Schema  | Create an index on all nodes with a particular label and a single property; i.e. create a single-property index.
|<<administration-indexes-create-a-named-composite-index, CREATE INDEX [composite] FOR (n:Label) ON (n.prop1, ..., n.propN)>>  | Schema  | Create an index on all nodes with a particular label and multiple properties; i.e. create a composite index.
...
|<<administration-constraints-drop-a-constraint-by-name, DROP CONSTRAINT name>>      | Schema   | Drop a constraint using the name.
|<<administration-indexes-drop-an-index-by-name, DROP INDEX name>>       | Schema | Drop an index using the name.
...
All of these commands are only executable against the system database.

[options="header"]
|===
|Command                                                                                                                                                                            | Category                      | Description
| <<administration-databases-show-databases, SHOW DATABASE>>                                                                                                                        | Database administration       | Returns information about a specified database.
| <<administration-databases-show-databases, SHOW DATABASES>>                                                                                                                       | Database administration       | Returns information about all databases.
| <<administration-databases-show-databases, SHOW DEFAULT DATABASE>>                                                                                                                | Database administration       | Returns information about the default database.
| <<administration-databases-create-database, CREATE [OR REPLACE] DATABASE ... [IF NOT EXISTS]>>                                                                                    | Database administration       | Crates a new database.
| <<administration-databases-drop-database, DROP DATABASE ... [IF EXISTS]>>                                                                                                         | Database administration       | Deletes a specified database.
| <<administration-databases-start-database, START DATABASE>>                                                                                                                       | Database administration       | Starts up a specified database.
| <<administration-databases-stop-database, STOP DATABASE>>                                                                                                                         | Database administration       | Stops a specified database.
| <<administration-security-roles-show, SHOW [ALL \| POPULATED] ROLES [WITH USERS]>>                                                                                                | User and role administration  | Returns information about all or populated roles, optionally including the assigned users.
| <<administration-security-roles-create, CREATE [OR REPLACE] ROLE ... [IF NOT EXISTS] ... [AS COPY OF]>>                                                                           | User and role administration  | Creates new roles.
| <<administration-security-roles-drop, DROP ROLE [IF EXISTS]>>                                                                                                                     | User and role administration  | Deletes a specified role.
| <<administration-security-users-show, SHOW USERS>>                                                                                                                                | User and role administration  | Returns information about all users.
| <<administration-security-users-create, CREATE [OR REPLACE] USER ... [IF NOT EXISTS] SET PASSWORD ... [SET PASSWORD CHANGE [NOT] REQUIRED] [SET STATUS [ACTIVE \| SUSPENDED]]>>   | User and role administration  | Creates a new user and sets the password for the new account. Optionally the account status can also be set and if the user should change the password upon first login.
| <<administration-security-users-drop, DROP USER ... [IF EXISTS]>>                                                                                                                 | User and role administration  | Deletes a specified user.
| <<administration-security-users-alter, ALTER USER ... [SET PASSWORD ...] [SET PASSWORD CHANGE [NOT] REQUIRED] [SET STATUS [ACTIVE \| SUSPENDED]]>>                                | User and role administration  | Changes a user account. Changes can include setting a new password, setting the account status and enabling that the user should change the password upon next login.
| <<administration-security-users-alter-password, ALTER CURRENT USER SET PASSWORD FROM ... TO>>                                                                                     | User and role administration  | Change the password of the user that is currently logged in.
| <<administration-security-subgraph-show, SHOW [ROLE ... \| USER ... \| ALL] PRIVILEGES>>                                                                                          | User and role administration  | Returns information about role, user or all privileges.
| <<administration-security-roles-grant, GRANT ROLE[S] ... TO>>                                                                                                                     | Privilege administration      | Assigns one or multiple roles to one or multiple users.
| <<administration-security-administration-database-privileges, GRANT ... ON DATABASE ... TO>>                                                                                      | Privilege administration      | Assigns a database or schema privilege to one or multiple roles.
| <<administration-security-subgraph-introduction, GRANT ... ON GRAPH ... [NODES \| RELATIONSHIPS \| ELEMENTS] ... TO>>                                                             | Privilege administration      | Assigns a graph privilege for one or multiple specified elements to one or multiple roles.
| <<tbd, GRANT ... ON DBMS TO>>  | Privilege administration  | Assigns a DBMS privilege to one or multiple roles.
| <<administration-security-administration-database-privileges, DENY ... ON DATABASE ... TO>>                                                                                       | Privilege administration      | Denies a database or schema privilege to one or multiple roles.
| <<administration-security-subgraph-introduction, DENY ... ON GRAPH ... [NODES \| RELATIONSHIPS \| ELEMENTS] ... TO>>                                                              | Privilege administration      | Denies a graph privilege for one or multiple specified elements to one or multiple roles.
| <<tbd, DENY ... ON DBMS TO>>  | Privilege administration  | Denies a DBMS privilege to one or multiple roles.
| <<administration-security-roles-revoke, REVOKE ROLE[S] ... FROM>>                                                                                                                 | Privilege administration      | Removes one or multiple roles from one or multiple users.
| <<administration-security-administration-database-privileges, REVOKE [GRANT \| DENY] ... ON DATABASE ... FROM>>                                                                   | Privilege administration      | Removes a database or schema privilege from one or multiple roles.
| <<administration-security-subgraph-revoke, REVOKE [GRANT \| DENY] ... ON GRAPH ... [NODES \| RELATIONSHIPS \| ELEMENTS] ... FROM>>                                                | Privilege administration      | Removes a graph privilege for one or multiple specified elements from one or multiple roles
| <<tbd, REVOKE [GRANT \| DENY] ... ON DBMS FROM>>  | Privilege administration  | Removes a DBMS privilege from one or multiple roles.
|===
...
| <<administration-security-administration-database-all, ALL DATABASE PRIVILEGES>>              | Database and schema   | Implies all privileges from the categories Database and Schema for a specific database.
| <<administration-security-subgraph-traverse, TRAVERSE>>                                       | GRAPH                 | Determines whether an element (node, relationship or both) can be found and traversed while executing queries on the specified graph.
| <<administration-security-subgraph-read, READ>>                                               | GRAPH                 | Determines whether the properties of an element (node, relationship or both) can be read while executing queries on the specified graph.
| <<administration-security-subgraph-match, MATCH>>                                             | GRAPH                 | Determines whether the properties of an element (node, relationship or both) can be read and the element can be found and traversed while executing queries on the specified graph.
| <<administration-security-subgraph-write, WRITE>>                                             | GRAPH                 | Determines whether the user can execute write operations on the specified graph.
| <<tbd, CREATE ROLE>>                                                                          | DBMS                  | Determines whether the user can create new roles.
| <<tbd, DROP ROLE>>                                                                            | DBMS                  | Determines whether the user can delete roles.
| <<tbd, ASSIGN ROLE>>                                                                          | DBMS                  | Determines whether the user can grant roles.
| <<tbd, REMOVE ROLE>>                                                                          | DBMS                  | Determines whether the user can revoke roles.
| <<tbd, SHOW ROLE>>                                                                            | DBMS                  | Determines whether the user can get information about existing and assigned roles.
| <<tbd, ROLE MANAGEMENT>>                                                                      | DBMS                  | Determines whether the user can create, drop, grant, revoke and show roles.
|===
