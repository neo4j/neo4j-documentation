...
| <<administration-security-users-alter, ALTER USER ... [SET PASSWORD {password [CHANGE [NOT] REQUIRED] | CHANGE [NOT] REQUIRED}] [SET STATUS {ACTIVE | SUSPENDED}]>>               | User and role administration  | Changes a user account. Changes can include setting a new password, setting the account status and enabling that the user should change the password upon next login.
...
| <<administration-security-roles-grant, GRANT ROLE[S] ... TO>>                                                                                                                     | User and role administration  | Assigns one or multiple roles to one or multiple users.
| <<administration-security-roles-revoke, REVOKE ROLE[S] ... FROM>>                                                                                                                 | User and role administration  | Removes one or multiple roles from one or multiple users.
| <<administration-security-subgraph-show, SHOW [ROLE ... \| USER ... \| ALL ] PRIVILEGES>>                                                                                         | Privilege administration      | Returns information about role, user or all privileges.
