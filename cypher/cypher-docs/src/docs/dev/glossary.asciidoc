...
|<<administration-constraints-syntax, CREATE OR REPLACE CONSTRAINT existence ON (n:Label) ASSERT exists(n.property)>>  | Schema   | Create or replace a constraint ensuring that all nodes with a particular label have a certain property.
...
|<<administration-constraints-syntax, CREATE OR REPLACE CONSTRAINT node_key ON (n:Label) ASSERT (n.prop1, ..., n.propN) IS NODE KEY>>  |  Schema | Create or replace a constraint ensuring all nodes with a particular label have all the specified properties and that the combination of property values is unique; i.e. ensures existence and uniqueness.
...
|<<administration-constraints-syntax, CREATE OR REPLACE CONSTRAINT existence ON ()-[r:REL_TYPE]-() ASSERT exists(r.property)>>  | Schema   | Create or replace a constraint ensuring that all relationships with a particular type have a certain property.
...
|<<administration-constraints-syntax, CREATE OR REPLACE CONSTRAINT uniqueness ON (n:Label) ASSERT n.property IS UNIQUE>>  |  Schema | Create or replace a constraint ensuring the uniqueness of the combination of node label and property value for a particular property key across all nodes.
...
|<<administration-indexes-syntax, CREATE OR REPLACE INDEX single FOR (n:Label) ON (n.property)>>  | Schema  | Create or replace an index on all nodes with a particular label and a single property; i.e. create a single-property index replacing any already existing index with that name.
...
|<<administration-indexes-syntax, CREATE OR REPLACE INDEX composite FOR (n:Label) ON (n.prop1, ..., n.propN)>>  | Schema  | Create or replace an index on all nodes with a particular label and multiple properties; i.e. create a composite index replacing any already existing index with that name.
