...
|<<functions-ceil, ceil()>>                     | Numeric           | Returns the smallest floating point number greater than or equal to a number.
...
|<<functions-exists,exists()>>                  | Predicate         | Returns true if a match for a pattern exists in the graph, or if a specified property exists in the node, relationship or map.
...
|<<functions-floor, floor()>>                   | Numeric           | Returns the largest floating point number less than or equal to a number.
