...
|<<functions-isnan, isNaN()>>                    | Numeric           | Returns whether a value is `NaN`.
...
| <<access-control-execute-boosted-user-defined-function, EXECUTE BOOSTED FUNCTION>>    | DBMS  | Determines whether the user get elevated privileges when executing functions.
| <<access-control-execute-boosted-procedure, EXECUTE BOOSTED PROCEDURE>>               | DBMS  | Determines whether the user get elevated privileges when executing procedures.
