...
| <<access-control-list-privileges, SHOW [ROLE ... \| USER ... \| ALL ] PRIVILEGES [AS [REVOKE] COMMAND[S]]>>                                                                                         | Privilege      | Returns information about role, user or all privileges.
...
| <<access-control-execute-procedure-subsection, EXECUTE ADMIN PROCEDURE>>                                         | DBMS                  | Determines whether the user can execute admin procedures.
| <<access-control-execute-boosted-user-defined-function, EXECUTE BOOSTED FUNCTION>>                                      | DBMS                  | Determines whether the user can execute functions with elevated privileges.
| <<access-control-execute-boosted-procedure, EXECUTE BOOSTED PROCEDURE>>                                      | DBMS                  | Determines whether the user can execute procedures with elevated privileges.
...
| <<access-control-privileges-reads-match, MATCH>>                                                          | GRAPH                 | Determines whether the properties of an element (node, relationship or both) can be read and the element can be found and traversed while executing queries on the specified graph.
