...
[IMPORTANT]
When using `MERGE` or `MATCH` with `LOAD CSV` we need to make sure we have an index (see <<cypher-manual#administration-indexes>>) or a unique constraint (see <<cypher-manual#administration-constraints>>) on the property we're merging.
