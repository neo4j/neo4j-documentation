...
For this query we'll use `CALL +++{ ... }+++ IN TRANSACTIONS` (see <<cypher-manual#subquery-call-in-transactions>>) which is helpful for queries that operate on large CSV files.
This is a way to periodically commit progress in a new transaction.
In this case we also set the limit to `500` rows per transaction.
