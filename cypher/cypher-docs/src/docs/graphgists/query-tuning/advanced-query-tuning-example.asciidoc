...
In Neo4j 3.4 and earlier, the fact that the index contains the property value, and the results are returned in a specific order, 
was not used improve the performance of any later part of the query that might depend on the property value or result order.
...
For information on native index support and upgrade considerations regarding indexes, see <<operations-manual#schema-indexes, Operations Manual -> Schema indexes>>.
====
...
We'll use a movies data set to illustrate this more advanced query tuning.
