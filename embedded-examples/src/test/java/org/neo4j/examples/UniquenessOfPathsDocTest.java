...
public class UniquenessOfPathsDocTest extends AbstractJavaDocTestBase {
    ...
    public void pathUniquenessExample(GraphDatabaseService graphDb) {
        Node start = data.get().get("Pet0");
        JavaTestDocsGenerator generator = gen.get();
        generator.addSnippet("graph", createGraphVizWithNodeId("Descendants example graph", graphDb, generator.getTitle()));
        generator.addTestSourceSnippets(this.getClass(), "traverser", "traverseNodeGlobal");
        // tag::traverser[]
        Node dataTarget = data.get().get("Principal1");
        String output = "";
        int count = 0;
        try (Transaction transaction = graphDb.beginTx()) {
            start = transaction.getNodeByElementId(start.getElementId());
            final Node target = transaction.getNodeByElementId(dataTarget.getElementId());
        ...
        generator.addSnippet("output", createOutputSnippet(output));
        assertEquals(2, count);

        String output2 = "";
        count = 0;
        try (Transaction tx = graphDb.beginTx()) {
            start = tx.getNodeByElementId(start.getElementId());
            final Node target = tx.getNodeByElementId(dataTarget.getElementId());
            // tag::traverseNodeGlobal[]
            TraversalDescription nodeGlobalTd = tx.traversalDescription().uniqueness(Uniqueness.NODE_PATH).evaluator(path -> {
                boolean endNodeIsTarget = path.endNode().equals(target);
                return Evaluation.of(endNodeIsTarget, !endNodeIsTarget);
        ...
        generator.addSnippet("outNodeGlobal", createOutputSnippet(output2));
